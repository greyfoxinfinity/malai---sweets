...
export default function CartPage() {
  ...
  function submitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const orderMessage = [
      `Hello ${businessName}! I would like to place a cash-on-delivery order.`,
      "",
      "ORDER",
      ...lines,
      "",
    ...
    setSent(true);
    setMessage(
      "Your order summary is open in WhatsApp. Send the message there to complete your order.",
    );
  }
    ...
    return (
      <main className="mx-auto flex min-h-[65vh] max-w-7xl flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-4 max-w-md text-black/60">
          Pick something delicious, then we&apos;ll help you send the order details on WhatsApp.
        </p>
        <Link
          href="/shop"
  ...
  return (
    <main className="bg-[#f7f5f1] px-6 py-12 md:px-10 md:py-20 lg:px-16">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          {user && (
            <p className="text-sm text-black/50">
              Ordering as{" "}
              <span className="font-medium text-black/70">{user.name}</span> ·{" "}
              <button
                type="button"
        ...
        {sent ? (
          <div className="mt-16 rounded-2xl bg-white p-10 text-center">
            ...
            <h2 className="mt-3 text-2xl font-semibold">Thank you!</h2>
            <p className="mt-4 max-w-md mx-auto text-black/60">{message}</p>
            <Link
              href="/shop"
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-[1.2fr_.8fr]">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex justify-between gap-4">
                      <div>
                        <p className="text-xs font-medium uppercase tracking-[.16em] text-black/45">
                          {item.category}
                        </p>
                      ...
                      <button
                        ...
                        className="h-11 w-11 rounded-full text-black/50 hover:bg-black/5"
                      >
            ...
            <aside className="h-fit rounded-2xl bg-white p-6 sm:p-7">
              ...
              <p className="mt-4 text-xs leading-5 text-black/40">
                {settings?.deliveryNote ||
                  "Delivery availability and charges are confirmed on WhatsApp before your order is accepted."}
              </p>
              ...
              {whatsappReady ? (
                <form onSubmit={submitOrder} className="mt-6 space-y-3">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full resize-none rounded-xl border border-black/10 bg-[#f7f5f1] px-4 py-3 text-sm outline-none focus:border-[#8b5e3c]"
                  />
                  <button
                    type="submit"
                    className="w-full rounded-full bg-[#25D366] px-5 py-3.5 text-sm font-semibold text-white transition-transform hover:scale-[1.02]"
                  >
                    Send order on WhatsApp
                  </button>
                </form>
              ) : (
                <p className="mt-6 rounded-xl bg-[#f7f5f1] p-4 text-sm text-black/50">
                  WhatsApp ordering is not configured yet. Please contact us directly.
                </p>
              )}
            </aside>
          </div>
